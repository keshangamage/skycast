export function SearchBar({
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 group">
      <div className="relative flex-1">
        ...
        <input
          ...
          className="relative w-full rounded-xl sm:rounded-2xl border border-white/30 bg-white/20 backdrop-blur-lg pl-10 sm:pl-12 pr-4 sm:pr-6 py-3 sm:py-4 text-base sm:text-lg text-white placeholder:text-white/60 shadow-2xl outline-none transition-all duration-300 hover:bg-white/25 hover:border-white/40 focus:bg-white/30 focus:border-white/50 focus:ring-4 focus:ring-white/20 focus:scale-[1.02]"
        />
      </div>
      <button
        onClick={onSearch}
        disabled={loading}
        className="relative inline-flex min-w-full sm:min-w-32 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-r from-white/20 to-white/10 backdrop-blur-lg px-4 sm:px-6 py-3 sm:py-4 text-base sm:text-lg font-semibold text-white shadow-2xl border border-white/30 transition-all duration-300 hover:from-white/30 hover:to-white/20 hover:border-white/40 hover:scale-105 focus:scale-105 focus:ring-4 focus:ring-white/20 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:scale-100 group overflow-hidden"
      >
